...
fn args() -> (String, HashMap<String, String>) {
    ...
    while i < raw.len() {
        if let Some(name) = raw[i].strip_prefix("--") {
            ...
            let value = if takes_value { next.cloned().unwrap_or_default() } else { String::new() };
...
fn load_genesis(path: &str, flags: &HashMap<String, String>) -> Result<Genesis, String> {
    ...
    let admin = flags.get("admin").cloned().ok_or_else(|| {
        format!("genesis file {} not found; pass --admin <inz address> to create one", path)
    })?;
    let g = Genesis::default_devnet(&admin);
    std::fs::write(path, serde_json::to_string_pretty(&g).unwrap()).map_err(|e| e.to_string())?;
    println!("[genesis] created {} with 1,000,000 INAZ to {}", path, admin);
    Ok(g)
}
...
fn run() -> Result<(), String> {
    let (cmd, flags) = args();
    let data = flags.get("data").cloned().unwrap_or_else(|| "./data".into());
    let genesis_path = flags.get("genesis").cloned().unwrap_or_else(|| "./genesis.json".into());
    ...
    match cmd.as_str() {
        ...
        "init" => {
            ...
            println!("  supply        {} INAZ", format_inaz(node.store.total_supply()));
            Ok(())
        }
        ...
        "run" => {
            ...
            let rpc_addr = flags.get("rpc").cloned().unwrap_or_else(|| "0.0.0.0:9933".into());
            ...
            let p2p_addr = flags.get("p2p").cloned().unwrap_or_else(|| "0.0.0.0:9944".into());
            ...
            println!(
                ...
                if network.allowed_ids.is_empty() { "off".to_string() } else { format!("{} keys", network.allowed_ids.len()) }
            );
            println!("block time   {} ms", block_time);
            println!("height       {}", node.store.tip_height().unwrap_or(0));
            println!("supply       {} INAZ", format_inaz(node.store.total_supply()));
            println!("staked       {} INAZ across {} validators", format_inaz(node.store.total_staked()), node.validators().len());
            println!("min stake    {} INAZ to validate", format_inaz(types::MIN_STAKE));
            println!("mode         {}", if replica { "replica (serving only)" } else if node.solo() { "solo (no peers)" } else { "networked" });
            println!("ws           {}", if ws_addr.is_empty() { "disabled".to_string() } else { format!("ws://{}", ws_addr) });
            println!("finalized    height {}", node.store.finalized_height());
            println!("base fee     {} rai (floor {} rai)", node.base_fee(), node.fee_floor());
            println!("state root   merkle from height {}", state::STATE_ROOT_V2_ACTIVATION_HEIGHT);
            ...
            if !stake_qos.is_empty() {
                println!("qos          {} keys weighted by stake (cap {}x)", stake_qos.len(), qos::MAX_STAKE_MULTIPLIER);
            }
            let rpc_cfg = Arc::new(rpcauth::RpcConfig::with_qos(
                key_list("rpc-keys", "INAZ_RPC_KEYS"),
                key_list("rpc-admin-keys", "INAZ_RPC_ADMIN_KEYS"),
                flags.contains_key("rpc-require-auth") || std::env::var("INAZ_RPC_REQUIRE_AUTH").is_ok(),
                flags.contains_key("rpc-trust-proxy") || std::env::var("INAZ_RPC_TRUST_PROXY").is_ok(),
                stake_qos,
            ));
                ...
                match node.produce_block() {
                    Ok(Some(b)) => {
                        if !b.transactions.is_empty() {
                            println!("[block] #{} txs={} hash={}", b.height, b.transactions.len(), &b.hash[..16]);
                        }
        ...
        "balance" => {
            let rpc_url = flags.get("rpc").cloned().unwrap_or_else(|| "http://127.0.0.1:9933".into());
            let address = flags.get("address").ok_or("--address required")?.clone();
            let res = rpc_call(&rpc_url, "inaz_getAccount", serde_json::json!({ "address": address }))?;
            println!("{} => {} INAZ (staked {}, unbonding {}, rewards {}, nonce {})",
                address,
        ...
        "validators" => {
            ...
            println!("total stake  {} INAZ", res["totalStakeInaz"].as_str().unwrap_or("0"));
            println!("min stake    {} INAZ", res["minStakeInaz"].as_str().unwrap_or("0"));
            println!("next leader  {}", res["nextLeader"].as_str().unwrap_or("(bootstrap)"));
        ...
        "finality" => {
            ...
            println!("queried height    {} (final: {})", res["height"], res["isFinal"]);
            println!("precommits        {} of {} validators", res["votes"], res["validators"]);
            println!("voted stake       {} INAZ ({}% of {} INAZ, need >66.67%)",
        ...
        "slashing" => {
            ...
            println!("enforcement         {} (from #{})",
                if res["active"].as_bool().unwrap_or(false) { "active" } else { "pending" },
                res["activationHeight"]);
            let p = &res["params"];
            println!("equivocation burn   max({}%, {}x stake share) of stake + permanent tombstone",
                p["equivocationMinBurnPct"], p["equivocationCorrelationFactor"]);
            println!("reporter bounty     {}% of the burn", p["reporterBountyPct"]);
            println!("downtime jail       {} consecutive missed slots -> {} blocks in jail",
                p["downtimeJailStreak"], p["downtimeJailBlocks"]);
            println!("total burned        {} INAZ across {} slashes",
                res["totalBurnedInaz"].as_str().unwrap_or("0"), res["slashCount"]);
        ...
        "report" => {
            let rpc_url = flags.get("rpc").cloned().unwrap_or_else(|| "http://127.0.0.1:9933".into());
            let path = flags.get("evidence").ok_or("--evidence <file.json> required")?;
            let text = std::fs::read_to_string(path).map_err(|e| e.to_string())?;
            let evidence: serde_json::Value =
                serde_json::from_str(&text).map_err(|e| format!("bad evidence file: {}", e))?;
            let preview = rpc_call(&rpc_url, "inaz_previewSlash", serde_json::json!({ "evidence": evidence }))?;
            println!("offender   {}", preview["offender"].as_str().unwrap_or(""));
            println!("offence    {} at #{}", preview["offence"].as_str().unwrap_or(""), preview["offenceHeight"]);
            println!("burn       {} INAZ ({}% of stake)", preview["burnInaz"].as_str().unwrap_or("0"), preview["burnPct"]);
            println!("bounty     {} INAZ", preview["bountyInaz"].as_str().unwrap_or("0"));
            let res = rpc_call(&rpc_url, "inaz_reportEquivocation", serde_json::json!({ "evidence": evidence }))?;
        ...
        "unjail" => {
            let rpc_url = flags.get("rpc").cloned().unwrap_or_else(|| "http://127.0.0.1:9933".into());
            let kp = Keypair::from_secret_hex(flags.get("key").ok_or("--key required")?)?;
            let acct = rpc_call(&rpc_url, "inaz_getAccount", serde_json::json!({ "address": kp.address() }))?;
            let nonce = acct["pendingNonce"].as_u64().or_else(|| acct["nonce"].as_u64()).unwrap_or(0);
            let fee = flags.get("fee").map(|f| parse_inaz(f)).transpose()?.unwrap_or(MIN_FEE);
            ...
            let res = rpc_call(&rpc_url, "inaz_sendTransaction", serde_json::json!({ "tx": tx }))?;
        ...
        "tokens" => {
            ...
            println!("creation fee  {} INAZ", res["creationFeeInaz"].as_str().unwrap_or("0"));
            if let Some(list) = res["tokens"].as_array() {
                for t in list {
                    println!(
                        ...
                        t["holders"],
                        if t["mintable"].as_bool().unwrap_or(false) { "mintable" } else { "fixed supply" },
                    );
                }
            }
            Ok(())
        ...
        "token-balance" => {
            let rpc_url = flags.get("rpc").cloned().unwrap_or_else(|| "http://127.0.0.1:9933".into());
            let address = flags.get("address").ok_or("--address required")?.clone();
            let res = rpc_call(&rpc_url, "inaz_tokenHoldings", serde_json::json!({ "address": address }))?;
            let empty = Vec::new();
        ...
        "token-create" | "token-mint" | "token-send" | "token-burn" => {
            ...
            let (kind, payload, amount, to) = match cmd.as_str() {
                "token-create" => {
                    let symbol = tokens::normalize_symbol(flags.get("symbol").ok_or("--symbol required")?)?;
                    ...
                    let p = Payload { token: String::new(), symbol, name, decimals, mintable, ..Default::default() };
                    (TxKind::CreateToken, Some(p), supply, kp.address())
                }
                _ => {
                    let token = flags.get("token").ok_or("--token required")?.clone();
                    let info = rpc_call(&rpc_url, "inaz_getToken", serde_json::json!({ "token": token }))?;
                    let decimals = info["decimals"].as_u64().unwrap_or(9) as u8;
                    let amount = tokens::parse_units(flags.get("amount").ok_or("--amount required")?, decimals)?;
                    let to = match cmd.as_str() {
                    ...
                    let p = Payload { token, ..Default::default() };
        ...
        "deploy" | "call" => {
            ...
            let (kind, to, payload) = if cmd == "deploy" {
                ...
                let code = if path.ends_with(".wat") {
                    wat::parse_bytes(&raw).map_err(|e| format!("bad wat: {}", e))?.to_vec()
                } else {
                    raw
                };
                let p = Payload { code: hex::encode(&code), ..Default::default() };
                println!("code {} bytes, deploy fee {} INAZ", code.len(), format_inaz(contracts::DEPLOY_FEE));
                (TxKind::DeployContract, kp.address(), Some(p))
            } else {
                let addr = flags.get("contract").ok_or("--contract ADDR required")?.clone();
                let args = flags.get("args").cloned().unwrap_or_default();
                (TxKind::CallContract, addr, Some(Payload { args, ..Default::default() }))
            };
            let mut tx = Transaction {
                kind,
                from_pubkey: kp.pubkey_hex(),
            ...
            if cmd == "deploy" {
                println!("contract:  {}", res["contract"].as_str().unwrap_or("(pending)"));
            }
        ...
        "query" => {
            let rpc_url = flags.get("rpc").cloned().unwrap_or_else(|| "http://127.0.0.1:9933".into());
            let address = flags.get("contract").ok_or("--contract ADDR required")?.clone();
            let args = flags.get("args").cloned().unwrap_or_default();
            let res = rpc_call(&rpc_url, "inaz_query", serde_json::json!({ "address": address, "args": args }))?;
            println!("ok         {}", res["ok"]);
        ...
        "receipt" => {
            ...
            let res = rpc_call(&rpc_url, "inaz_getReceipt", serde_json::json!({ "hash": hash }))?;
        ...
        "bench" => {
            let rpc_url = flags.get("rpc").cloned().unwrap_or_else(|| "http://127.0.0.1:9933".into());
            let kp = Keypair::from_secret_hex(flags.get("key").ok_or("--key required")?)?;
            let count: u64 = flags.get("count").and_then(|c| c.parse().ok()).unwrap_or(1000);
            let batch: usize = flags.get("batch").and_then(|c| c.parse().ok()).unwrap_or(500);
            ...
            println!("signed     {} txs in {:.0} ms ({:.0} sig/s client side)", count, sign_ms, count as f64 * 1000.0 / sign_ms);
            ...
            for chunk in txs.chunks(batch.max(1)) {
                match rpc_call(&rpc_url, "inaz_sendTransactions", serde_json::json!({ "txs": chunk })) {
            ...
            println!("submitted  {}/{} txs in {:.0} ms ({:.0} tx/s into mempool)", ok, count, submit_ms, ok as f64 * 1000.0 / submit_ms);
            ...
            loop {
                ...
                let info = rpc_call(&rpc_url, "inaz_chainInfo", serde_json::json!({}))?;
                let done = info["totalTxs"].as_u64().unwrap_or(0).saturating_sub(start_txs);
                ...
                if (done >= ok && mempool == 0) || timeout {
                    ...
                    let blocks = info["height"].as_u64().unwrap_or(0).saturating_sub(start_height);
                    println!("executed   {} txs across {} blocks in {:.0} ms", done, blocks, ms);
                    println!("throughput {:.0} tx/s (executed and finalised)", done as f64 * 1000.0 / ms);
                    if blocks > 0 {
                        println!("density    {} txs per block avg", done / blocks);
                    }
                    if timeout {
                        println!("note       stopped at the 120s cap, {} still queued", mempool);
                    }
                    break;
                }
            }
        ...
        _ => {
            ...
            println!("  bench   --key HEX --count N               submit N txs and measure throughput");
            Ok(())
        }
    }
}
